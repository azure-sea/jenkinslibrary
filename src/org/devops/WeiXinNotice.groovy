...
// 依赖插件 "HTTP Request" 与 "build user vars" 
// 使用方法:
// 成功 WeiXinNotice.HttpReq("xxx平台发布",code_branch_or_tag,"构建成功 ✅")
// 失败 WeiXinNotice.HttpReq("xxx平台发布",code_branch_or_tag,"构建失败 ❌",CatchInfo)
// 中断 WeiXinNotice.HttpReq("xxx平台发布",code_branch_or_tag,"构建失败 ❌","暂停或中断")
def getChangeString() {
    def changeString = ""
    def MAX_MSG_LEN = 20
...
def HttpReq(Deploy_env,code_branch_or_tag,Status,CatchInfo=' '){
    wrap([$class: 'BuildUser']){
        def WeiXinHook = "https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=xxx"
